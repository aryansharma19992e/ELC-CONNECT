...
export default function UserDashboard() {
	...
	useEffect(() => {
		...
		try {
			const cached = getItem('dashboardStats')
			if (cached) {
		...
		async function load() {
			try {
				...
				try { setItem('dashboardStats', JSON.stringify(computed)) } catch { }
		...
		load()
	}, [router, isClient]) // Removed getItem, setItem, clear from dependencies
